...
def run_match(p1, p2, play_fun, render_mode, k, lam, kwargs):

    x = p1.get_elo()
    y = p2.get_elo()
    
    result = play_fun(players=[p1, p2], render_mode=render_mode, **kwargs)
...
def parallel_round(players, matchmaking_fun, play_fun, render_mode : str, k : int, lam : int, n_jobs=-1, **kwargs):
    """
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(run_match)(p1, p2, play_fun, render_mode, k, lam, kwargs)
...
def round(players : list[ind.Individual], matchmaking_fun,  play_fun, render_mode : str, k : int, lam : int, **kwargs):
    """
...
def play(player_class = ind.RandomIndividual, matchmaking_fun = mmk.matches, play_fun = cns.play_boxing, parallel = True, **kwargs):
    ...
    for r in tqdm(range (number_of_rounds), desc="Tournament on going", unit="round"):
        if parallel:
            parallel_round(players = players, matchmaking_fun= matchmaking_fun, play_fun = play_fun, render_mode = render_mode, k = k, lam = lam, n_jobs=-1, **kwargs) #fixed to used the maximum number of jobs
        else:
            round(players = players, matchmaking_fun= matchmaking_fun, play_fun = play_fun, render_mode = render_mode, k = k, lam = lam, **kwargs)
