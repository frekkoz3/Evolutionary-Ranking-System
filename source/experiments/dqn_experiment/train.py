...
if __name__ == '__main__':
    ...
    p1 = GNGDQNAgent(DQNAgent(n_actions, n_observations), DQNAgent(n_actions, n_observations))
    p2 = GNGDQNAgent(DQNAgent(n_actions, n_observations), DQNAgent(n_actions, n_observations))
    ...
    for i in range (number_of_iterations):
        ...
        for j in tqdm(range (n_games), desc=f"Iteration {i} on going", unit="game"):
            p1.update_t = 0
            p2.update_t = 0
            play_grab_n_go(players=[p1, p2], render_mode=render_mode, eval_mode= False)
            play_grab_n_go(players=[p2, p1], render_mode=render_mode, eval_mode= False)
        p1.reset()
        p2.reset()
        # --------------------------------------
        # Checkpoints saving
